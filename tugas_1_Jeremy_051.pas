program tugas_1_Jeremy_051;
uses crt,sysutils;
var 
   nama,nim,stambuk,kodeprodi,fakultas,kodefakultas,prodi,jalur,nomorurut:string;
   nimint:integer;
   kelas:char;
...
begin
   clrscr;
      ...
      write('Masukkan NIM anda='); read(nim);
         clrscr;
            nimint:=strtoint(nim);
               stambuk:=nim[1]+nim[2];
                     ...
                     case (nimint) of
                        ...
                        nimint:=strtoint(prodi);
                           case (nimint) of 
                              ...
                              0310:kodeprodi:='Agroteknologi (PDSKU)';
                              0401:kodeprodi:='Teknik Mesin';
                              0402:kodeprodi:='Teknik Elektro';
                              0403:kodeprodi:='Teknik Sipil';
                              0404:kodeprodi:='Teknik Industri';
                              ...
                              nimint:=strtoint(nomorurut);
                                 if (nimint mod 3=1) then begin
                                    ...
                                    kelas:='B';
                                 end else if (nimint mod 3=0) then begin
                                    kelas:='C';
                                 end;
                                    if (nimint<=34) then begin
                                       jalur:='SNBP';
                                    end else if (nimint<=75) then begin
                                       jalur:='SNBT';
                                    end else begin
                                       jalur:='Mandiri';
                                    end;
                                       nimint:=strtoint(fakultas);
                                       if (nimint=14) then begin
                                          writeln('Nama     :',nama);
                                          writeln('NIM      :',nim);
